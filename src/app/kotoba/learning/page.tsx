...
export default function LearningKotobaSelectionPage() {
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {Array.from({ length: totalParts }, (_, i) => i + 1).map((part) => (
            <Link key={part} href={`/kotoba/learning/${part}`} passHref>
              <Card className="flex transform cursor-pointer flex-col justify-between transition-transform duration-300 hover:scale-105 hover:shadow-xl">
                ...
                <CardContent>
                  <p className="text-muted-foreground">
                    Kosakata {(part - 1) * ITEMS_PER_PART + 1} - {Math.min(part * ITEMS_PER_PART, flashcards.length)}
                  </p>
                </CardContent>
              </Card>
            </Link>
