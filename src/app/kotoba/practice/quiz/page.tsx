  ...
  useEffect(() => {
    if (selectedParts.length > 0) {
      ...
      selectedParts.forEach((part) => {
        ...
        questions = [...questions, ...flashcards.slice(startIndex, endIndex)];
      });
      ...
      const generatedQuizCards = questions.map((card) => {
        const incorrectAnswers = flashcards
          .filter((f) => f.front !== card.front)
          .sort(() => 0.5 - Math.random())
          .slice(0, NUM_CHOICES - 1)
          .map((f) => f.front);
        
        const choices = [card.front, ...incorrectAnswers].sort(() => 0.5 - Math.random());
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-center text-lg">
              Skor Anda:
              <p className="my-4 text-5xl font-bold text-primary">{Math.round((score / quizCards.length) * 100)}</p>
              Benar {score} dari {quizCards.length} pertanyaan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex-col gap-2 sm:flex-row">
            <Button variant="outline" onClick={() => router.push('/kotoba/practice')}>
              Pilih Bagian Lain
            </Button>
...
export default function KotobaQuizPage() {
    return (
