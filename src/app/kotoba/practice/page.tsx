...
export default function PracticeKotobaSelectionPage() {
  const router = useRouter();
  const totalParts = Math.ceil(flashcards.length / ITEMS_PER_PART);
  ...
  const startPractice = () => {
    if (selectedParts.length > 0) {
      ...
      router.push(`/kotoba/practice/quiz?parts=${partsQuery}`);
    }
  };
            ...
            >
              ...
              <CardContent>
                <p className="text-muted-foreground">
                  Kosakata {(part - 1) * ITEMS_PER_PART + 1} -{" "}
                  {Math.min(part * ITEMS_PER_PART, flashcards.length)}
                </p>
              </CardContent>
            </Card>
          ))}
