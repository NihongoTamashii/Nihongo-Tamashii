...
const kanaTypes: { [key: string]: { title: string; list: Kana[] } } = {
  gojuon: { title: "Gojuon (Vokal Dasar)", list: katakana.gojuon.filter(k => k.character) },
  dakuon: { title: "Dakuon", list: katakana.dakuon },
  handakuon: { title: "Handakuon", list: katakana.handakuon },
  youon: { title: "Youon", list: katakana.youon },
};

export default function LearningKatakanaPage({ params }: { params: Promise<{ type: string }> }) {
  const { type } = React.use(params);
  
  const { title, list: flashcards } = useMemo(() => {
    return kanaTypes[type] || { title: 'Katakana', list: [] };
  }, [type]);
