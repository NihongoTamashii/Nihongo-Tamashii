...
import { madeParticle } from "@/lib/data/particles";
import { Separator } from "@/components/ui/separator";

export default function ParticleMadePage() {
  ...
  return (
    <div className="flex h-screen w-full flex-col bg-background">
      ...
      <main className="flex-1 space-y-6 overflow-y-auto p-6">
        <Card>
            <CardHeader>
                <CardTitle className="font-headline text-2xl">{madeParticle.title}</CardTitle>
                <CardDescription>
                {madeParticle.meaning}
                </CardDescription>
            </CardHeader>
        </Card>
        
        {madeParticle.usages.map((usage, index) => (
          <Card key={index}>
            <CardHeader>
